function ArrayBuffer(byte_length) {
  //byte unsigned int
  ...
  function set(index, value) {
    ...
    if (typeof value != 'number') {
      for (x in value) {
        ...
        this.buffer[(index + this.begin) * 4] = parseInt(
          bit_str.substring(bit_str.length - 8, bit_str.length),
          2,
        );
        this.buffer[(index + this.begin) * 4 + 1] = parseInt(
          bit_str.substring(bit_str.length - 16, bit_str.length - 8),
          2,
        );
        this.buffer[(index + this.begin) * 4 + 2] = parseInt(
          bit_str.substring(bit_str.length - 24, bit_str.length - 16),
          2,
        );
        this.buffer[(index + this.begin) * 4 + 3] = parseInt(
          bit_str.substring(bit_str.length - 36, bit_str.length - 24),
          2,
        );
        index++;
      }
...
function Int8Array(ArrayBuffer_Object) {
  ...
  function get(index) {
    ...
    bit_str = (parseInt(BitInversion(this.buffer[index + this.begin].toString(2)), 2) + 1).toString(
      2,
    );
